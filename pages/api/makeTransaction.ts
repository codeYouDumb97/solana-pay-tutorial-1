...
import { couponAddress, shopAddress, usdcToken } from "../../lib/addresses"
...
async function post(
  ...
  try {
    ...
    const usdcMint = await getMint(connection, usdcToken)
    // Get the buyer's USDC token account address
    const buyerUsdcAddress = await getAssociatedTokenAddress(usdcToken, buyerPublicKey)
    // Get the shop's USDC token account address
    const shopUsdcAddress = await getAssociatedTokenAddress(usdcToken, shopPublicKey)
